...
import { downloadAndReplaceImages } from "./download-images.js";
...
function parseArgs(argv: string[]): Args {
  const args: Args = { url: "", wait: false, timeout: DEFAULT_TIMEOUT_MS, downloadImages: true };
...
async function main(): Promise<void> {
  ...
  if (!args.url) {
    console.error("Usage: bun main.ts <url> [-o output.md] [--wait] [--no-images] [--timeout ms]");
    process.exit(1);
  }
  ...
  if (args.downloadImages) {
    ...
    const imgResult = await downloadAndReplaceImages(finalDocument, imagesDir);
    finalDocument = imgResult.markdown;
    if (imgResult.downloaded > 0) {
      console.log(`Downloaded ${imgResult.downloaded} images${imgResult.failed > 0 ? ` (${imgResult.failed} failed)` : ""}`);
    }
  }
  await writeFile(outputPath, finalDocument, "utf-8");
